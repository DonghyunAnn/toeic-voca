...
**3,159단어 / 30 DAY** — 예문 3,159개(전부), 발음 3,155개.
...
| 엑셀 교재 (`.xlsx`) | 티어 구분 (필수 어휘 / 토익 만점 완성) |

셋 다 같은 교재이고 단어는 3,155개로 일치한다. 블로그는 그중 '필수 어휘' 단만
예문과 함께 올렸다. 남은 1,574단어의 예문은 만들어 채웠고 `생성` 표시가 붙는다.

대조 과정과 세 자료에서 찾은 오류는 `docs-spec/`의 두 문서에 있다.
...
xlsx_tier.py          엑셀 교재에서 티어(필수/만점완성) 추출
...
교재는 단어를 두 단으로 나눈다. **필수 어휘** 1,663개와 **토익 만점 완성** 1,496개다.
설정의 **단어 등급**으로 나눠서 공부할 수 있고, 목록 화면에도 같은 필터가 있다.
필수 어휘를 먼저 끝내는 것이 순서다.
